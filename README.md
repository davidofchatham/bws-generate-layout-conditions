...
| Featured Image Slot Active (theme) | GeneratePress itself is drawing the featured image on this page. False when a Page Hero or Content Template has taken it over, when a Layout Element has switched it off, when the Customizer's global image toggle is off, and on every non-singular page. Says nothing about whether a thumbnail exists | — (deliberately no class) | `featured-image-active` (different — requires a thumbnail) |
