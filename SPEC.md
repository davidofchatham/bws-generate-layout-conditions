...
- V23 `GeneratePress_Conditions::show_data()` requires array-of-arrays for all three args. `get_post_meta(...,true)` returns `''` when unset — always normalize with `?: array()` before passing. Raw empty string → `in_array($val,'')` → fatal `TypeError`.
...
| B3 | 2026-06-02 | `layout_element_disables()` passed raw `get_post_meta()` return values to `GeneratePress_Conditions::show_data()` — `get_post_meta` returns `''` when meta unset; `show_data` expects array-of-arrays; `in_array($val, '')` → fatal `TypeError` on any Layout Element with no display/exclude/user conditions set | Normalize with `?: array()` before passing (V4) |
