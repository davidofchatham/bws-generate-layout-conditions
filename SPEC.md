...
- ~~**T8 V22 gap** — featured-image config-replay on non-singular.~~ **Landed, then REVERSED by ADR-0006 (2026-08-06.)** The off-singular replay is gone: its only source was `_generate_disable_featured_image`, which on real sites marks a relocation rather than a disable. `featured_image_active` is now the per-post metabox alone, and a constant off singular. See V22.
