...
| T13 | . | V2 poisoned-signal proof | V2, V20 | Fixtures `ls-el-header-block` / `ls-el-footer-block` exist and seed, but nothing yet asserts the poisoning itself — that a Block Element on `generate_header`/`generate_footer` unconditionally `remove_action`s the native construct, so hook-state reads "disabled" on every page carrying it regardless of intent. This is the observation config-replay (V2) exists to route around; currently the invariant is documented, not executable. Assert via the seam's `has_hook()` against a real request on `ls-page-poisoned` vs `ls-page-baseline`. |
