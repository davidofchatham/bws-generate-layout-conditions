...
| Featured image | Hybrid: hook-state (singular) + config-replay (non-singular) | singular: `has_action('generate_after_entry_header','generate_blog_single_featured_image')`; non-singular: layout `_generate_disable_featured_image` | Page Hero ambiguity (V21); the hook read covers one of three Customizer positions and requires the Blog module (V33); the page-header image path is unmodeled (V33); post-metabox layer absent off-singular by design (ADR-0002) | GP only adds hook on `is_singular()` (V20/B2); archive detection via replay since T8 (V22). The singular branch is the last hook-state signal with a known-wrong direction — deferred entry in `ROADMAP.md` |
