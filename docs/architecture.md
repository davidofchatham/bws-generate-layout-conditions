...
| Secondary nav | Config-replay (post-meta + Layout Element) | post: `_generate-disable-secondary-nav`; layout: `_generate_disable_secondary_navigation` | None — no Customizer secondary-nav disable layer | Hook signal unreadable by construction, not merely absent: array-callback on `has_nav_menu`, and the filter exists to make "disabled" look like "unassigned" (V32). Replay branch ungated on `is_singular()` — GP's filter is too. Element key is `_navigation`, metabox key is `-nav`; they are not the same word |
