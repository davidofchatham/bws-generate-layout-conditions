...
**Detector**:
The plugin's single source of truth for current disable state. Both the body-class consumer and the condition consumer read from it. Hybrid: config-replay for header, footer, content title and secondary nav; hook-state for primary nav, top bar, and featured image on singular. The replay list has only ever grown — treat any remaining hook-state signal as provisional (V32, V33). Lazy + memoized — full resolution runs at most once per request (V5). (Mechanism: ADR-0001, ADR-0005, V5.)
