...
It reads hook state, which the disable signals no longer do, and that is not a relapse — the question *is* about hook state, so relocation removing the callback is the true answer rather than a poisoned one. It still says nothing about pixels: it never consults `has_post_thumbnail()`, which is why the name is "slot active" and not "rendered". **Nested inside the post-setting rule**: this plugin removes GP's five image callbacks when the per-post toggle is set, so the slot rule is false whenever that rule is false, and "post setting disabled, slot active" cannot occur. No body class. (Contract: V7, V11, V34. Rationale: architecture.md, GitHub issue #4.)
