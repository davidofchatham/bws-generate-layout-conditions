...
**Detector**:
The plugin's single source of truth for current disable state. Both the body-class consumer and the condition consumer read from it. Config-replay for header, footer, content title and secondary nav; per-post metabox alone for the featured image (ADR-0006); hook-state only for primary nav and top bar. The hook-state list has only ever shrunk and the replay list has only ever grown — treat both remaining hook reads as provisional (V32, V33). Lazy + memoized — full resolution runs at most once per request (V5). (Mechanism: ADR-0001, ADR-0005, ADR-0006, V5.)
