...
The place GeneratePress itself draws the featured image. **Featured Image Slot Active (theme)** reports whether that slot is live on this request — a different question from the post-setting rule above, about a different subject: the theme's wiring rather than the author's checkbox. False under a Page Hero or Content Template relocation, under a Layout Element kill switch, and on every non-singular request (the native singular slot does not exist there). Also false when the Customizer's global image toggle is off — but only where GP Premium's **Blog** module is active, which is both where that toggle lives and where the module's own gating makes hook state carry it. Without that module the theme's pair is registered at file scope, so the rule is coarser: true on any singular page nothing has explicitly switched off. See V34 part 5 for the two false-active directions that leaves. Used with `is_not` to avoid drawing a duplicate image from a block, or with `is` inverted to supply a fallback banner where nothing native is being drawn.
