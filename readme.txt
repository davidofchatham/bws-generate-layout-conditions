...
* Note: there is no condition for "is GeneratePress itself drawing a featured image here?" A block wanting to avoid duplicating GP's image has no rule to key on. Same trade as the title rule: the previous behaviour failed by hiding blocks with nothing on screen to explain why, this one fails at worst by showing a duplicate you can see.
