...
* Added: a second featured-image rule, **Featured Image Slot Active (theme)**, answering the question the one above deliberately does not: is GeneratePress itself drawing the featured image on this page? It is false when a Page Hero or Content Template has taken the image over, when a Layout Element has switched it off, when the Customizer's global image toggle is off, and on every non-singular page. Use it with *is not* to stop a block of your own duplicating GeneratePress' image, or to draw a fallback banner where nothing native is being drawn. Like every other rule it reports configuration, not pixels — on a post with no thumbnail it is still true, so a fallback image is not conditioned away. It adds no `body` class.
